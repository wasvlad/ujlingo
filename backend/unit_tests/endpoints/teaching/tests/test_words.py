...
class TestWordsTesting:
    ...
    async def test_normal_flow(self, client):
        result = client.post("/teaching/tests/words/new")
        assert result.status_code == 200
        assert result.json() == {"message": "Test session initialized"}
        while True:
            result = client.get("/teaching/get_question")
            assert result.status_code == 200
            if result.json().get("message", None) is not None:
                break
            assert result.json().get("question", None) is not None
            result = client.post("/teaching/answer_question",
